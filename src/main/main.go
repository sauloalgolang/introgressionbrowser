...
func processFile(sourceFile string, isTar bool, isGz bool) {
...
func processVcf(r io.Reader) {
	...
	// 			fmt.Print("Integer GetGenotypeField ")
	// 			fmt.Println(variant.GetGenotypeField(variant.Samples[0], "PL", int(-1)))
	// 		} else {
	// 			fmt.Print("Other   GetGenotypeField ")
	...
	// if _, ok := header.SampleFormats["PL"]; ok {
	// 	if header.SampleFormats["PL"].Type == "Integer" {
	// 		hasPL = true
	// 		plFmt = int(-1)
	// 	} else if header.SampleFormats["PL"].Type == "Float" {
	// 		hasPL = true
	// 		plFmt = float32(-1)
	// 	} else {
	// 		hasPL = false
