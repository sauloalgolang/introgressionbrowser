...
func NewDistanceMatrix1D64(chromosomeName string, blockSize uint64, blockPosition uint64, blockNumber uint64, dimension uint64) *DistanceMatrix1D64 {
	...
	r := DistanceMatrix1D64{
		...
		Dimension:      dimension,
		Size:           size,
		Data:           make(DistanceRow64, size, size),
	}
...
func (d *DistanceMatrix1D64) GenFilename(outPrefix string, format string, compression string) (baseName string, fileName string) {
...
func (d *DistanceMatrix1D64) Save(outPrefix string, format string, compression string) {
...
func (d *DistanceMatrix1D64) Load(outPrefix string, format string, compression string) {
	baseName, _ := d.GenFilename(outPrefix, format, compression)
